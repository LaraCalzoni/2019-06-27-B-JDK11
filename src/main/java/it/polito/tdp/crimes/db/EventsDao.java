...
public class EventsDao {
	...
	public List <String> getVertici(String categoria, int mese){
		...
		try {
			...
			while(res.next()) {
					...
					if(!result.contains(res.getString("offense_type_id"))) {
	...
	public List <Adiacenza> getAdiacenze(String categoria, int mese){
		String sql="SELECT e1.offense_type_id AS reato1, e2.offense_type_id AS reato2, COUNT(DISTINCT(e1.neighborhood_id)) AS peso "
				+ "FROM events e1, events e2 "
				+ "WHERE e1.offense_category_id= ? AND e2.offense_category_id= e1.offense_category_id AND "
				+ "MONTH(e1.reported_date)= ? AND MONTH(e2.reported_date) = MONTH(e1.reported_date) "
		...
		try {
			...
			while(res.next()) {
					...
					result.add(new Adiacenza(res.getString("reato1"), res.getString("reato2"), res.getInt("peso")));
